  ...
  if(posIDs.find(bID) != posIDs.end()){
    ...
    if(b.x > 0 and b.y > 0){
      ...
      if(!supBlocks(b)){
        ...
        posIDs.erase(secondBlock->kind);//bloc que ha sigut borrat ja no esta apuntat per el ID que l'apuntaba
        return true;
      }
    }
  }
...
*/
bool push(int posVec, AST *secondBlockRoot){
  int sx, sy, sh, sw, height;
  tblock secBlock;
  if(secondBlockRoot->kind != "list"){
    //Si ens han donat una ID i no existia o que comparteix blocs superiors, no fem res
    if(posIDs.find(secondBlockRoot->kind) == posIDs.end() or supBlocks(secBlock)) return false;
  ...
  //Comprovem que podem possar el bloc
...
if(posIDs.find(IDb) != posIDs.end()){
  ...
  //Si el bloc es a la graella
  if(b.x > 0 and b.y > 0){
    int h, w;//tamany del bloc a comprovar
          ...
          if(g.height[i][j] < l){
            ...
            for(int ci = 0; !conflict and i+ci < b.y+b.w-1 and ci < w; ++ci){
              for(int cj = 0; !conflict and j+cj < b.x+b.h-1 and cj < h; ++cj){
                ...
                if(g.height[i+ci][j+cj] != g.height[i][j]) conflict = true;
              }
            }
            if(!conflict) return true;//Si arriva al final sense conflictes, hem trovat un lloc on possarlo
          }
        }
      }
    }
...
#token NUM "[0-9]+"
#token ID "[B] [0-9]*"
#token FUNC "[a-zA-Z] [a-zA-Z0-9]*"
//#token COMMENT "[a-zA-Z0-9\-\(\)\\\"\'\[\]\;\.\,\\&\|]*"
...
defs: (functionDef <<#0=createASTlist(_sibling);>>)*;

functionDef: DEF^ FUNC ops ENDEF!;

instruction:
  ID ASIG^ expr
  | MOVE^ ID DIRECTION NUM
  | WHILE^ LP! boolexpr RP! LSB! ops RSB!
  | hexpr
  | FUNC^;

expr:
  PLACE^ pxyp AT! pxyp
  | ( (ID (PUSH^ | POP^)) | (pxyp PUSH^) ) pexpr;
...
boolexpr:
  ( (FITS^ LP! ID COMA! xy COMA! NUM RP!) | ((hexpr | NUM) COMP^ (hexpr | NUM)) ) { AND^ boolexpr };

hexpr: HEIGHT^ LP! ID RP!;

pxyp: LP! xy RP!;
xy: NUM COMA! NUM <<#0=createASTlist(_sibling);>>;
