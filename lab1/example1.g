...
int evaluate(AST *a) {
  if (a == NULL) return 0;
    ...
    if ( m.find(a->text) == m.end() ) {
      cout << "var  \"" + a->text + "\" does not exist." << endl;
      exit(1);
    } else {
      return m[a->text];
    }
...
void execute(AST *a) {
  if (a == NULL) return;
  else if (a->kind == ":=") m[child(a,0)->text] = evaluate(child(a,1));
  else // a->kind == "write"
...
program: (instruction)* "@";
instruction: (WRITE^ expr) | (ID ASIG^ expr) | (expr COMP^ expr);
expr: term (PLUS^ term)*;
